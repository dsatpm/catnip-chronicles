...
const BossBattle = () => {
  ...
  const updateHealthDisplay = () => {
    ...
    updateHealthBar('player', 'Lucy Purr', 'player-health', 'player-health-bar');
  ...
  const updateHealthBar = (entity, name, healthId, barId) => {
    ...
    const barElement = document.getElementById(barId);
		const health = Math.min(5, Math.max(0, entity === 'player' ? playerHealth : bossHealth));

    
    if (healthElement && barElement) {
      healthElement.innerText = `${name}: ${entity === 'player' ? playerHealth : bossHealth}`;
  ...
  const endGame = (winner) => {
    ...
    const overlay = createOverlay('Congratulations!<br>You have bested DreadClaw! He Has Become DEADCLAW');
